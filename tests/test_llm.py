...
class TestLLMPrompt(unittest.TestCase):
    ...
    """
    async def test_get_llm_answer_clean_text(self):
        """Test the get_llm_answer function with cleaned text."""
        question = 'How are you? :smiley:'
        expected_clean_text = 'How are you? '
        with patch('llm_integration.llm.llm_prompt', new_callable=unittest.mock.AsyncMock) as mock_llm_prompt:
            mock_llm_prompt.return_value = "Response"
            await get_llm_answer(question)
            mock_llm_prompt.assert_called_once_with(question=expected_clean_text)

    async def test_get_llm_answer_clean_text_quotes(self):
    ...
    async def test_get_llm_answer_clean_text_slack_markup(self):
    ...
    async def test_get_llm_answer_all(self):
